...
import { installUnwind, unwindPersistently, stillArmed } from "./unwind.js";
...
async function main() {
  ...
  if (process.env.ARM === "1" && subNow === 0n) {
    ...
    // Read it BACK. A mined subscribe is not an armed subscription, and beat 3
    // failing for want of one looks like the Deadhand not working.
    if (!(await stillArmed())) throw new Error("subscribeTo mined but nothing is armed — beat 3 will NOT fire");
